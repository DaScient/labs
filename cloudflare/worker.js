// Cloudflare Worker — clean start
// Endpoints: /api/ping, /api/summary?symbol=AAPL, /api/options?symbol=AAPL, /api/mock?symbol=AAPL
export default {
  async fetch(request) {
    ...
    try {
      switch (url.pathname) {
        case "/api/ping":   return json({ ok: true, now: Date.now() });
        case "/api/mock": {  // always works (for UI testing)
          const symbol = (url.searchParams.get("symbol") || "AAPL").toUpperCase();
          return json({ symbol, price: 123.45, targetMeanPrice: 140.0, recommendationKey: "buy",
            options: { expiration: "2025-09-19",
              atmCall:{ strike:125, mid:4.20 }, atmPut:{ strike:120, mid:3.80 },
